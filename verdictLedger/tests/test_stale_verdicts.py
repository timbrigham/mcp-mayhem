...
def test_a_fail_against_different_bytes_demotes_to_stale(ledger):
    """⭐ THE HEADLINE REGRESSION. `build` failed on sha 'old'; we are asking about
    sha 'new'. The honest status is "never run on this", not "this failed"."""
    inv = _build(ledger, [_rec("build", "old", "FAIL")], {"README.md": "new"})
    row = next(r for r in inv["rows"] if r["step"] == "build")
...
def test_a_fail_against_these_bytes_is_still_a_fail(ledger):
    ...
    inv = _build(ledger, [_rec("build", "same", "FAIL")], {"README.md": "same"})
    row = next(r for r in inv["rows"] if r["step"] == "build")
    assert row["status"] == "FAIL"


def test_an_undecided_against_other_bytes_also_demotes(ledger):
    inv = _build(ledger, [_rec("build", "old", "UNDECIDED")], {"README.md": "new"})
    assert next(r for r in inv["rows"] if r["step"] == "build")["status"] == "STALE"
...
def test_demoting_weakens_no_gate(ledger):
    ...
    inv = _build(ledger, [_rec("build", "old", "FAIL"), _rec("check_prose", "new", "PASS")],
                 {"README.md": "new"}, admission=["build"])
...
def test_a_covering_record_wins_over_a_stale_one(ledger):
    ...
    records = [
        {"id": "build@old#0", "step": "build", "verdict": "FAIL", "revision": 0,
         "decided": {"how": "mechanical", "passes": 1, "agreed": 1},
         "subjects": [{"path": "README.md", "sha256": "old"}],
         "basis": {"kind": "tree", "value": "old"}},
        {"id": "build@new2#0", "step": "build", "verdict": "PASS", "revision": 0,
         "decided": {"how": "mechanical", "passes": 1, "agreed": 1},
         "subjects": [{"path": "GUIDE.md", "sha256": "new2"}],
         "basis": {"kind": "tree", "value": "new2"}},
    ]
    row = next(r for r in _build(ledger, records, files)["rows"] if r["step"] == "build")
    assert row["status"] == "STALE"          # partly examined, not FAILED
    assert row["record_id"] == "build@new2#0"   # the COVERING record speaks
...
def test_stale_does_not_count_as_examined(ledger):
    ...
    inv = _build(ledger, [_rec("build", "old", "PASS")], {"README.md": "new"})
